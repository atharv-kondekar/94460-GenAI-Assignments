...
if not st.session_state.logged_in:
    ...
    menu = st.sidebar.radio(f"Welcome, {st.session_state.userid}", ["Explore CSV", "See History", "Logout"])
...
if not st.session_state.logged_in:
    ...
    if menu == "Home":
        ...
        if login_btn:
            users = load_users()
            valid = users[(users["userid"] == userid) & (users["password"] == password)]
            if not valid.empty:
    ...
    if menu == "Explore CSV":
        ...
        df = pd.read_csv(HISTORY_FILE)
        user_history = df[df["userid"] == st.session_state.userid]
